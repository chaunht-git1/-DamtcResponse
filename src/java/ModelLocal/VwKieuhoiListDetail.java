package ModelLocal;

import java.math.BigDecimal;
import java.sql.Blob;
import java.util.Date;

public class VwKieuhoiListDetail {

    private String maqlds;
    private Number sobn;
    private String sophieu;
    private Number stt;
    private String idchinhanh;
    private String chinhanhname;
    private String iddoitac;
    private String doitacname;
    private Date ngaynhaplieu;
    private String datruyenfile;
    private String idnvkt;
    private String chinhanhnhaplieu;
    private String hoten;
    private String cmnd;
    private String socttuythan;
    private String manguoinhan;
    private String idloaitiengoi;
    private String idloaitienchitra;
    private String idloaicongno;
    private String nguoigoi;
    private BigDecimal sotiengoi;
    private BigDecimal tygia;
    private BigDecimal thanhtien;
    private String diachi;
    private String dienthoai;
    private String iddiadiemchitra;
    private String idnvnhaplieu;
    private String ghichugoi;
    private String dakiemtra;
    private Number countprint;
    private String iddistrict;
    private String districtname;
    private String idcity;
    private String cityname;
    private Number solaninds;
    private String dachitra;
    private String nghingo;
    private String tentat;
    private Blob hinhbn;
    private String biennhanc;
    private String agent;
    private String biennhan;
    private Date ngaybiennhan;
    private String payagentseq;
    private Number dothb;
    private Date ngayhoibao;
    private String khoa;
    private Number fileno;
    private String daxuatfile;
    private Date ngaychitra;
    private String hbdt;
    private String kiemtra;
    private String sobn2;
    private String dotbn;
    private Date ngaykt;
    private Date datemodified;
    private Date ngaybndc;
    private Blob hinhdc;
    private String dieuchinh;
    private String ttagent;
    private Date ngayxuatfile;
    private String idnvxuatfile;
    private Number dot;
    private Number dotbiennhan;
    private String idagent;
    private String agenttn;
    private Number dotmail;
    private String congtacvien;
    private String dachitradetail;
    private String diadiemchitradetail;
    private String ghichu;
    private String duongdanimage;
    private String checkimage;
    private Boolean disimage;

    public String getGhichu() {
        return ghichu;
    }

    public void setGhichu(String ghichu) {
        this.ghichu = ghichu;
    }

    public Boolean getDisimage() {
        return disimage;
    }

    public void setDisimage(Boolean disimage) {
        this.disimage = disimage;
    }

    public String getCheckimage() {
        return checkimage;
    }

    public void setCheckimage(String checkimage) {
        this.checkimage = checkimage;
    }

    public String getDuongdanimage() {
        return duongdanimage;
    }

    public void setDuongdanimage(String duongdanimage) {
        this.duongdanimage = duongdanimage;
    }

    public String getDiadiemchitradetail() {
        return diadiemchitradetail;
    }

    public void setDiadiemchitradetail(String diadiemchitradetail) {
        this.diadiemchitradetail = diadiemchitradetail;
    }

    public String getDachitradetail() {
        return dachitradetail;
    }

    public void setDachitradetail(String dachitradetail) {
        this.dachitradetail = dachitradetail;
    }

    public String getMaqlds() {
        return maqlds;
    }

    public void setMaqlds(String maqlds) {
        this.maqlds = maqlds;
    }

    public Number getSobn() {
        return sobn;
    }

    public void setSobn(Number sobn) {
        this.sobn = sobn;
    }

    public String getSophieu() {
        return sophieu;
    }

    public void setSophieu(String sophieu) {
        this.sophieu = sophieu;
    }

    public Number getStt() {
        return stt;
    }

    public void setStt(Number stt) {
        this.stt = stt;
    }

    public String getIdchinhanh() {
        return idchinhanh;
    }

    public void setIdchinhanh(String idchinhanh) {
        this.idchinhanh = idchinhanh;
    }

    public String getChinhanhname() {
        return chinhanhname;
    }

    public void setChinhanhname(String chinhanhname) {
        this.chinhanhname = chinhanhname;
    }

    public String getIddoitac() {
        return iddoitac;
    }

    public void setIddoitac(String iddoitac) {
        this.iddoitac = iddoitac;
    }

    public String getDoitacname() {
        return doitacname;
    }

    public void setDoitacname(String doitacname) {
        this.doitacname = doitacname;
    }

    public Date getNgaynhaplieu() {
        return ngaynhaplieu;
    }

    public void setNgaynhaplieu(Date ngaynhaplieu) {
        this.ngaynhaplieu = ngaynhaplieu;
    }

    public String getDatruyenfile() {
        return datruyenfile;
    }

    public void setDatruyenfile(String datruyenfile) {
        this.datruyenfile = datruyenfile;
    }

    public String getIdnvkt() {
        return idnvkt;
    }

    public void setIdnvkt(String idnvkt) {
        this.idnvkt = idnvkt;
    }

    public String getChinhanhnhaplieu() {
        return chinhanhnhaplieu;
    }

    public void setChinhanhnhaplieu(String chinhanhnhaplieu) {
        this.chinhanhnhaplieu = chinhanhnhaplieu;
    }

    public String getHoten() {
        return hoten;
    }

    public void setHoten(String hoten) {
        this.hoten = hoten;
    }

    public String getCmnd() {
        return cmnd;
    }

    public void setCmnd(String cmnd) {
        this.cmnd = cmnd;
    }

    public String getSocttuythan() {
        return socttuythan;
    }

    public void setSocttuythan(String socttuythan) {
        this.socttuythan = socttuythan;
    }

    public String getManguoinhan() {
        return manguoinhan;
    }

    public void setManguoinhan(String manguoinhan) {
        this.manguoinhan = manguoinhan;
    }

    public String getIdloaitiengoi() {
        return idloaitiengoi;
    }

    public void setIdloaitiengoi(String idloaitiengoi) {
        this.idloaitiengoi = idloaitiengoi;
    }

    public String getIdloaitienchitra() {
        return idloaitienchitra;
    }

    public void setIdloaitienchitra(String idloaitienchitra) {
        this.idloaitienchitra = idloaitienchitra;
    }

    public String getIdloaicongno() {
        return idloaicongno;
    }

    public void setIdloaicongno(String idloaicongno) {
        this.idloaicongno = idloaicongno;
    }

    public String getNguoigoi() {
        return nguoigoi;
    }

    public void setNguoigoi(String nguoigoi) {
        this.nguoigoi = nguoigoi;
    }

    public BigDecimal getSotiengoi() {
        return sotiengoi;
    }

    public void setSotiengoi(BigDecimal sotiengoi) {
        this.sotiengoi = sotiengoi;
    }

    public BigDecimal getTygia() {
        return tygia;
    }

    public void setTygia(BigDecimal tygia) {
        this.tygia = tygia;
    }

    public BigDecimal getThanhtien() {
        return thanhtien;
    }

    public void setThanhtien(BigDecimal thanhtien) {
        this.thanhtien = thanhtien;
    }

    public String getDiachi() {
        return diachi;
    }

    public void setDiachi(String diachi) {
        this.diachi = diachi;
    }

    public String getDienthoai() {
        return dienthoai;
    }

    public void setDienthoai(String dienthoai) {
        this.dienthoai = dienthoai;
    }

    public String getIddiadiemchitra() {
        return iddiadiemchitra;
    }

    public void setIddiadiemchitra(String iddiadiemchitra) {
        this.iddiadiemchitra = iddiadiemchitra;
    }

    public String getIdnvnhaplieu() {
        return idnvnhaplieu;
    }

    public void setIdnvnhaplieu(String idnvnhaplieu) {
        this.idnvnhaplieu = idnvnhaplieu;
    }

    public String getGhichugoi() {
        return ghichugoi;
    }

    public void setGhichugoi(String ghichugoi) {
        this.ghichugoi = ghichugoi;
    }

    public String getDakiemtra() {
        return dakiemtra;
    }

    public void setDakiemtra(String dakiemtra) {
        this.dakiemtra = dakiemtra;
    }

    public Number getCountprint() {
        return countprint;
    }

    public void setCountprint(Number countprint) {
        this.countprint = countprint;
    }

    public String getIddistrict() {
        return iddistrict;
    }

    public void setIddistrict(String iddistrict) {
        this.iddistrict = iddistrict;
    }

    public String getDistrictname() {
        return districtname;
    }

    public void setDistrictname(String districtname) {
        this.districtname = districtname;
    }

    public String getIdcity() {
        return idcity;
    }

    public void setIdcity(String idcity) {
        this.idcity = idcity;
    }

    public String getCityname() {
        return cityname;
    }

    public void setCityname(String cityname) {
        this.cityname = cityname;
    }

    public Number getSolaninds() {
        return solaninds;
    }

    public void setSolaninds(Number solaninds) {
        this.solaninds = solaninds;
    }

    public String getDachitra() {
        return dachitra;
    }

    public void setDachitra(String dachitra) {
        this.dachitra = dachitra;
    }

    public String getNghingo() {
        return nghingo;
    }

    public void setNghingo(String nghingo) {
        this.nghingo = nghingo;
    }

    public String getTentat() {
        return tentat;
    }

    public void setTentat(String tentat) {
        this.tentat = tentat;
    }

    public Blob getHinhbn() {
        return hinhbn;
    }

    public void setHinhbn(Blob hinhbn) {
        this.hinhbn = hinhbn;
    }

    public String getBiennhanc() {
        return biennhanc;
    }

    public void setBiennhanc(String biennhanc) {
        this.biennhanc = biennhanc;
    }

    public String getAgent() {
        return agent;
    }

    public void setAgent(String agent) {
        this.agent = agent;
    }

    public String getBiennhan() {
        return biennhan;
    }

    public void setBiennhan(String biennhan) {
        this.biennhan = biennhan;
    }

    public Date getNgaybiennhan() {
        return ngaybiennhan;
    }

    public void setNgaybiennhan(Date ngaybiennhan) {
        this.ngaybiennhan = ngaybiennhan;
    }

    public String getPayagentseq() {
        return payagentseq;
    }

    public void setPayagentseq(String payagentseq) {
        this.payagentseq = payagentseq;
    }

    public Number getDothb() {
        return dothb;
    }

    public void setDothb(Number dothb) {
        this.dothb = dothb;
    }

    public Date getNgayhoibao() {
        return ngayhoibao;
    }

    public void setNgayhoibao(Date ngayhoibao) {
        this.ngayhoibao = ngayhoibao;
    }

    public String getKhoa() {
        return khoa;
    }

    public void setKhoa(String khoa) {
        this.khoa = khoa;
    }

    public Number getFileno() {
        return fileno;
    }

    public void setFileno(Number fileno) {
        this.fileno = fileno;
    }

    public String getDaxuatfile() {
        return daxuatfile;
    }

    public void setDaxuatfile(String daxuatfile) {
        this.daxuatfile = daxuatfile;
    }

    public Date getNgaychitra() {
        return ngaychitra;
    }

    public void setNgaychitra(Date ngaychitra) {
        this.ngaychitra = ngaychitra;
    }

    public String getHbdt() {
        return hbdt;
    }

    public void setHbdt(String hbdt) {
        this.hbdt = hbdt;
    }

    public String getKiemtra() {
        return kiemtra;
    }

    public void setKiemtra(String kiemtra) {
        this.kiemtra = kiemtra;
    }

    public String getSobn2() {
        return sobn2;
    }

    public void setSobn2(String sobn2) {
        this.sobn2 = sobn2;
    }

    public String getDotbn() {
        return dotbn;
    }

    public void setDotbn(String dotbn) {
        this.dotbn = dotbn;
    }

    public Date getNgaykt() {
        return ngaykt;
    }

    public void setNgaykt(Date ngaykt) {
        this.ngaykt = ngaykt;
    }

    public Date getDatemodified() {
        return datemodified;
    }

    public void setDatemodified(Date datemodified) {
        this.datemodified = datemodified;
    }

    public Date getNgaybndc() {
        return ngaybndc;
    }

    public void setNgaybndc(Date ngaybndc) {
        this.ngaybndc = ngaybndc;
    }

    public Blob getHinhdc() {
        return hinhdc;
    }

    public void setHinhdc(Blob hinhdc) {
        this.hinhdc = hinhdc;
    }

    public String getDieuchinh() {
        return dieuchinh;
    }

    public void setDieuchinh(String dieuchinh) {
        this.dieuchinh = dieuchinh;
    }

    public String getTtagent() {
        return ttagent;
    }

    public void setTtagent(String ttagent) {
        this.ttagent = ttagent;
    }

    public Date getNgayxuatfile() {
        return ngayxuatfile;
    }

    public void setNgayxuatfile(Date ngayxuatfile) {
        this.ngayxuatfile = ngayxuatfile;
    }

    public String getIdnvxuatfile() {
        return idnvxuatfile;
    }

    public void setIdnvxuatfile(String idnvxuatfile) {
        this.idnvxuatfile = idnvxuatfile;
    }

    public Number getDot() {
        return dot;
    }

    public void setDot(Number dot) {
        this.dot = dot;
    }

    public Number getDotbiennhan() {
        return dotbiennhan;
    }

    public void setDotbiennhan(Number dotbiennhan) {
        this.dotbiennhan = dotbiennhan;
    }

    public String getIdagent() {
        return idagent;
    }

    public void setIdagent(String idagent) {
        this.idagent = idagent;
    }

    public String getAgenttn() {
        return agenttn;
    }

    public void setAgenttn(String agenttn) {
        this.agenttn = agenttn;
    }

    public Number getDotmail() {
        return dotmail;
    }

    public void setDotmail(Number dotmail) {
        this.dotmail = dotmail;
    }

    public String getCongtacvien() {
        return congtacvien;
    }

    public void setCongtacvien(String congtacvien) {
        this.congtacvien = congtacvien;
    }

}
