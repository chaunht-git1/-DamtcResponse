package ModelLocal;

import java.util.Date;

public class VwLichsuGdListWeb {

    private String mabiennhan;
    private String loai;
    private String lydo;
    private String tennv;
    private Date giodieuchinh;
    private String idchinhanh;
    private String sophieu;
    private String stt;
    private String sobn;
    private String iddoitac;
    private String chargio;

    public String getChargio() {
        return chargio;
    }

    public void setChargio(String chargio) {
        this.chargio = chargio;
    }
 
    
    

    public String getMabiennhan() {
        return mabiennhan;
    }

    public void setMabiennhan(String mabiennhan) {
        this.mabiennhan = mabiennhan;
    }

    public String getLoai() {
        return loai;
    }

    public void setLoai(String loai) {
        this.loai = loai;
    }

    public String getLydo() {
        return lydo;
    }

    public void setLydo(String lydo) {
        this.lydo = lydo;
    }

    public String getTennv() {
        return tennv;
    }

    public void setTennv(String tennv) {
        this.tennv = tennv;
    }

    public Date getGiodieuchinh() {
        return giodieuchinh;
    }

    public void setGiodieuchinh(Date giodieuchinh) {
        this.giodieuchinh = giodieuchinh;
    }

    public String getIdchinhanh() {
        return idchinhanh;
    }

    public void setIdchinhanh(String idchinhanh) {
        this.idchinhanh = idchinhanh;
    }

    public String getSophieu() {
        return sophieu;
    }

    public void setSophieu(String sophieu) {
        this.sophieu = sophieu;
    }

    public String getStt() {
        return stt;
    }

    public void setStt(String stt) {
        this.stt = stt;
    }

    public String getSobn() {
        return sobn;
    }

    public void setSobn(String sobn) {
        this.sobn = sobn;
    }

    public String getIddoitac() {
        return iddoitac;
    }

    public void setIddoitac(String iddoitac) {
        this.iddoitac = iddoitac;
    }
}
